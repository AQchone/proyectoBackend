...
class Controller {
  ...
  profile(req, res) {
    try {
      const isLoggedIn = req.cookies.accessToken !== undefined;
      const adminOptions =
        req.user.rol === "admin" || req.user.rol === "superAdmin";
      if (isLoggedIn) {
        const cartId = req.user.cart;
        const user = {
        ...
        return res
          .status(403)
          .json({ Error: "Debe logearse para poder acceder." });
      }
    } catch (err) {
  ...
  async sendMailToResetPassword(req, res) {
    try {
      ...
      const tokenPass =
        await this.#userRepository.sendMailToResetPassword(email);
      res.cookie("passToken", tokenPass, {
        maxAge: 60 * 60 * 1000,
        httpOnly: true,
      });
  ...
  async uploadDocuments(req, res) {
    try {
      ...
      const user = await this.#userRepository.updateUserDocuments(
        userId,
        files
      );
